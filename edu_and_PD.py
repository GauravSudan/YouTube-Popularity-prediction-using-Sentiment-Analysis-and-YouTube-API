...
#Visualising the train set result
    plt.scatter(d,test, color = 'red')        
    plt.plot(d, regressor.predict(d), color = 'blue')   #For line(PREDICTED)
...
# Visualising the SVR results (for higher resolution and smoother curve)
    ...
    plt.title('EDUCATION AND PERSONALITY DEVELOPMENT')
    plt.xlabel('Dates')
    plt.ylabel('Views/day')
    plt.show()
    
    ##################
    X_grid = np.arange(min(d), max(d), 0.01) # choice of 0.01 instead of 0.1 step because the data is feature scaled
    X_grid = X_grid.reshape((len(X_grid), 1))
    plt.scatter(d, test, color = 'red')
    plt.plot(X_grid, regressor.predict(X_grid), color = 'blue')
    plt.title('EDUCATION AND PERSONALITY DEVELOPMENT')
